...
function recolor([r,g,b,a]){const lum=(r+g+b)/765;return[Math.min(255,Math.round(r*0.66+37+lum*20)),Math.min(255,Math.round(g*0.57+19+lum*11)),Math.min(255,Math.round(b*0.78+55+lum*28)),a]}
function drawMask(out,maskX,maskY,attack,death){const gold=[218,171,72,255],goldHi=[255,220,122,255],dark=[84,50,39,255],voidCol=[39,27,42,255];
  ...
  tri(out,[maskX-rx+5,maskY-15],[maskX-rx-13-attack*6,maskY-31],[maskX-15,maskY-22],gold,1);tri(out,[maskX+rx-5,maskY-15],[maskX+rx+13+attack*6,maskY-31],[maskX+15,maskY-22],gold,1);
  line(out,maskX-15,maskY+13,maskX,maskY+22,dark,1);line(out,maskX,maskY+22,maskX+15,maskY+13,dark,1);line(out,maskX-8,maskY+18,maskX-4,maskY+25,goldHi,1,.75);line(out,maskX+8,maskY+18,maskX+4,maskY+25,goldHi,1,.75);
  if(attack>0){const radius=43+Math.round(attack*16);ellipse(out,maskX,maskY,radius,radius*.73,goldHi,.08+.13*attack);for(let a=0;a<12;a++){const ang=a*Math.PI/6;line(out,maskX+Math.cos(ang)*(radius-8),maskY+Math.sin(ang)*(radius*.73-6),maskX+Math.cos(ang)*(radius+5),maskY+Math.sin(ang)*(radius*.73+4),goldHi,1,.58+.28*attack)}}
  if(death>.35){line(out,maskX-24,maskY-18,maskX-6,maskY-3,voidCol,2,.75);line(out,maskX+18,maskY-13,maskX+4,maskY+2,voidCol,2,.72)}
}
function render(frame,motion,index,count){const out=Buffer.alloc(W*H*4),p=index/Math.max(1,count-1),sway=Math.sin(index*Math.PI/2),attack=motion==='attack'?Math.sin(p*Math.PI):0,death=motion==='death'?p:0,knock=motion==='knockback'?p:0;
  const bodyDx=motion==='move'?Math.round(sway*2):motion==='knockback'?-Math.round(p*8):0,bodyDy=motion==='death'?Math.round(p*10):4;
  const maskDx=motion==='move'?Math.round(sway*3):motion==='knockback'?-Math.round(p*3):0;
  const maskX=72+maskDx,maskY=40+Math.round(Math.cos(index*.9)*2)+(motion==='attack'?-Math.round(attack*4):0)+Math.round(death*38);
  ...
  const tether=[215,170,72,255];line(out,maskX-23,maskY+23,69+bodyDx,79+bodyDy,tether,1,.34);line(out,maskX+23,maskY+23,86+bodyDx,77+bodyDy,tether,1,.34);
  if(motion==='knockback'&&p>.2){line(out,maskX+28,maskY-8,maskX+40,maskY-9,tether,1,.38)}
  return out}
const delay=(ms)=>new Promise(r=>setTimeout(r,ms));
async function fetchBytes(url){let last;for(let attempt=1;attempt<=3;attempt++){const c=new AbortController(),to=setTimeout(()=>c.abort(),15000);try{const r=await fetch(url,{headers:{'user-agent':'frontline-summoners-build/3.0'},signal:c.signal});if(!r.ok)throw new Error(`HTTP ${r.status}`);return Buffer.from(await r.arrayBuffer())}catch(error){last=error;if(attempt<3)await delay(400*attempt)}finally{clearTimeout(to)}}throw new Error(`download failed ${url}: ${String(last)}`)}

await rm(dir,{recursive:true,force:true});await mkdir(dir,{recursive:true});
const meta=JSON.parse(await readFile(META,'utf8'));meta.targets['enemy-boss']={assetId:'unit:enemy-boss',sourceFamily:'evil-wizard',displayHeight:228,attackContactFrame:5,structuralRework:true,visualPolish:{version:GENERATOR_VERSION,kind:'GOLDEN_MASK_MOTION_LANGUAGE_PASS',reviewStatus:'UNREVIEWED_RUNTIME_FILES'},motions:{}};
...
console.log('[production art] enemy-boss golden-mask v3 materialized with delayed-mask knockback and mask-first death; lifecycle remains AWAITING_ART');
