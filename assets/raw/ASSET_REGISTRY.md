...
모든 외부 에셋은 사용 전에 원출처와 라이선스를 확인하고 기록한다. 런타임 미러는 원저작자/원라이선스를 대체하지 않는다.
...
| `luizmelo-evil-wizard` | `https://luizmelo.itch.io/evil-wizard` | LuizMelo | CC0 1.0 | Idle, Move, Attack | 코드상 tint/scale만 | Evil Wizard. 현재 런타임 미러는 `NQM765/IngeSoft1`의 동일 원본 파일을 사용. |

## 운영 원칙

- 기준 화풍은 LuizMelo 계열 CC0 횡스크롤 픽셀아트다.
- 다른 화풍의 무료 에셋을 단순히 수량 채우기 목적으로 섞지 않는다.
- 외부 미러는 개발 초기의 파일 전달 경로일 뿐이다. 안정화 단계에서 검증된 원본을 저장소/R2 등 정본 에셋 저장 위치로 옮긴다.
