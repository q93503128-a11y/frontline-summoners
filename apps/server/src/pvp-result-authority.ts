...
async function completeCasual1v1(
  ...
  if (match.state === 'COMPLETED') {
    return { matchId, result: match.result ?? result, modeId: match.mode_id, rated: false };
  }
  ...
  const aDelta = recordDelta(result, 'A');
  const bDelta = recordDelta(result, 'B');
  ...
  const writes = await db.batch([
    ...
    ).bind(aDelta.wins, aDelta.losses, aDelta.draws, now, a.user_id, aRating.revision),
    db.prepare(
      `UPDATE pvp_ratings SET
         casual_wins = casual_wins + ?1,
         casual_losses = casual_losses + ?2,
         ...
         updated_at = ?4
       WHERE user_id = ?5 AND revision = ?6`,
    ).bind(bDelta.wins, bDelta.losses, bDelta.draws, now, b.user_id, bRating.revision),
  ]);
