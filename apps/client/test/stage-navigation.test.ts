...
test('stage navigation covers four main chapters plus eleven executable SPECIAL collections', () => {
  assert.equal(STAGE_COLLECTIONS.length, 15);
  ...
  assert.equal(getStageCollectionForStage('special_glutton_04').id, 'special_glutton_dragon_nest');
  assert.equal(getStageCollectionForStage('special_echoes_03').id, 'special_three_echoes');
});
...
test('hub and collection paging remain bounded at current content volume', () => {
  ...
  assert.equal(getStageCollectionPageCount(), 8);
  ...
  assert.equal(getCollectionStagePageCount(getStageCollection('special_three_echoes')), 1);
});
...
test('permanent challenge collections enforce both chapter gates and internal NORMAL_CLEAR order', () => {
  assert.equal(isSortieStageUnlocked('special_glutton_01', CH1, []), true);
  ...
  assert.equal(isSortieStageUnlocked('special_undead_01', [...CH1, ...CH2.slice(0, 4)], []), false);
  assert.equal(isSortieStageUnlocked('special_undead_01', [...CH1, ...CH2.slice(0, 5)], []), true);
  assert.equal(isSortieStageUnlocked('special_glass_01', [...CH1, ...CH2, ...CH3.slice(0, 2)], []), false);
  assert.equal(isSortieStageUnlocked('special_glass_01', [...CH1, ...CH2, ...CH3.slice(0, 3)], []), true);
  assert.equal(isSortieStageUnlocked('special_mechcastle_01', [...CH1, ...CH2, ...CH3, ...CH4.slice(0, 4)], []), true);
  assert.equal(isSortieStageUnlocked('special_anomaly_01', [...CH1, ...CH2, ...CH3, ...CH4.slice(0, 5)], []), true);
