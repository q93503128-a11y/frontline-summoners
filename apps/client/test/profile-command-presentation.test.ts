...
test('profile presentation guard keeps loading copy but sanitizes internal failures', async () => {
  const presentation = await readSource('../src/profile-command-scene.ts');
  assert.match(presentation, /if \(\/불러오는 중\/\.test\(value\)\) return value/);
  assert.match(presentation, /HTTP_\|fetch\|network\|state hash\|revision\|request\|account_\|profile_/);
