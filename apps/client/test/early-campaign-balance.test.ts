...
test('stages one through five are beatable in the real sequential unlock and permanent reward order', () => {
  ...
  for (let stageIndex = 0; stageIndex < 5; stageIndex += 1) {
    ...
    assert.equal(ownedPermanentRewardIds.length, clearedStageIds.length, 'baseline must use exactly the permanent rewards earned from prior clears');
