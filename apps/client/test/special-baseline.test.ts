...
test('first SPECIAL pack is deterministically clearable by a completed chapter-one account', () => {
  const results = SPECIAL_STAGES.map((_, index) => autoPlaySpecialStage(index, 720));
  assert.equal(results.length, 5);

  for (const [index, result] of results.entries()) {
    const stage = SPECIAL_STAGES[index]!;
    assert.equal(
      result.state.battle.winner,
      'PLAYER',
      `${stage.id} ${stage.name} must be clearable by the deterministic completed-account baseline`,
    );
