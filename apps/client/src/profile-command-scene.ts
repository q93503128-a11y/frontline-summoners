...
function safeProfileText(value: string): string {
  ...
  const direct: Readonly<Record<string, string>> = {
    '지휘관 전과 기록': '지휘관 기록',
    '대표 장식과 누적 전과를 한 장의 복무 기록으로 정리한다.': '대표 장식과 전과를 한 장의 복무 기록으로 정리합니다.',
    '전선 복무 기록': '복무 기록',
    '대표 배지 · 표창대': '대표 표창',
    '장식 변경': '복무표 편집',
    ...
    const visit = (object: Phaser.GameObjects.GameObject): void => {
      if (object instanceof Phaser.GameObjects.Rectangle) {
        if (object.width >= 700 && object.height >= 430) {
          object.setFillStyle(0x171f28, 0.42).setStrokeStyle(1, 0x657080, 0.18);
        } else if (object.width >= 620 && object.height <= 2) {
          object.setAlpha(0.55);
        }
      }
      if (object instanceof Phaser.GameObjects.Text) {
        if (object.text.startsWith('업적 · ')) object.setAlpha(0.92);
        if (object.text === '복무 기록') object.setAlpha(0.82);
      }
