...
function rewriteStoryLine(value: string): string {
  const direct: Readonly<Record<string, string>> = {
    '선택형 스토리 · 전투 규칙과 해금 정보는 시스템 UI에 별도 표시됩니다.': 'SPACE / ENTER 진행 · ESC 건너뛰기',
    '다 음': '다음',
  };
...
export class StoryScene extends BaseStoryScene {
  ...
  private polishNarrativeFrame(): void {
    ...
    if (dialogue) dialogue.setFillStyle(0x151c27, 0.96).setStrokeStyle(2, 0x76859b, 0.62);
    for (const child of this.children.list) {
      if (child instanceof Phaser.GameObjects.Text && child.text.includes('SPACE / ENTER')) child.setAlpha(0.58);
    }
  }
}
