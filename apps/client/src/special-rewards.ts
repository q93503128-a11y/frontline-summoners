...
const ALL_REWARD_IDS = [...PERIODIC_REWARDS.map((reward) => reward.stageId), ...REWARDS.map((reward) => reward.stageId)];
...
export const PERIODIC_SPECIAL_REWARDS = PERIODIC_REWARDS;
...
for (const collectionId of PERIODIC_REWARD_COLLECTION_IDS) {
  if (!PERIODIC_REWARDS.some((reward) => reward.collectionId === collectionId)) throw new Error(`periodic reward collection has no stages: ${collectionId}`);
}
