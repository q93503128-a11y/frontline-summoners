...
export type { PermanentRewardApplicableSlot, PermanentRewardBattleInput, PermanentRewardBattleOutput, PermanentRewardDefinition, PermanentRewardModifier };
const KNOWN_SCOPES = new Set<string>(['ALL', ...REWARD_SCOPES]);
const KNOWN_KINDS = new Set<string>(['UNIT_HP_PERCENT','UNIT_ATTACK_PERCENT','STARTING_SUPPLY_PERCENT','PLAYER_BASE_HP_PERCENT','KILL_SUPPLY_PERCENT','WORKER_COST_REDUCTION_PERCENT','RECHARGE_REDUCTION_PERCENT','CHAPTER_FLAG']);
...
function parseModifier(value: unknown, context: string): PermanentRewardModifier {
  if (!isRecord(value) || typeof value.kind !== 'string' || !KNOWN_KINDS.has(value.kind)) throw new Error(`${context}.kind is unknown: ${String(isRecord(value) ? value.kind : undefined)}`);
...
const allRewards = [
  ...parseDefinitions(chapterOneRewardsJson, 'chapterOne'),
...
export function applyPercent(value: number, percent: number, minimum = 1): number { return applyPercentShared(value, percent, minimum); }
