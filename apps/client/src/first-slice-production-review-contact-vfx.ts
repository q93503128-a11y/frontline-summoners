...
function createContactObjects(
  ...
  y: number,
  direction: 1 | -1,
  tier: FirstSliceReviewCrowdTier,
  ...
  if (familyKey === 'militia_f2') {
    return [
      scene.add.rectangle(x + direction * 24, y - 2, tier === 'SATURATED' ? 68 : 86, 3, 0xd8e4dc, 0.88 * alpha).setDepth(14),
      scene.add.triangle(x + direction * (tier === 'SATURATED' ? 59 : 70), y - 2, -8 * direction, -6, -8 * direction, 6, 9 * direction, 0, 0xf4ead0, 0.94 * alpha).setDepth(15),
    ];
  }
...
function contactMotion(familyKey: FirstSliceReviewFamilyKey, tier: FirstSliceReviewCrowdTier): readonly [number, number, number] {
  if (familyKey === 'militia_f1') return [tier === 'OPEN' ? 150 : 118, 1.22, 1.16];
  if (familyKey === 'militia_f2') return [tier === 'OPEN' ? 135 : 105, 1.16, 1.05];
  if (familyKey === 'militia_f3') return [tier === 'OPEN' ? 126 : 100, 1.2, 1.08];
  if (familyKey === 'enemy-raider') return [tier === 'OPEN' ? 165 : 125, 1.38, 1.18];
  return [tier === 'OPEN' ? 260 : 205, tier === 'SATURATED' ? 1.42 : 1.75, tier === 'SATURATED' ? 1.28 : 1.55];
}
...
export function playFirstSliceReviewContactFx(
  ...
  const objects = createContactObjects(scene, familyKey, x, y, direction, tier);
  const [duration, scaleX, scaleY] = contactMotion(familyKey, tier);
  fadeAndDestroy(scene, objects, duration, scaleX, scaleY);
}

/** Static OPEN-density signature used by the non-authoritative capture board. */
export function drawFirstSliceReviewContactPreview(
  scene: Phaser.Scene,
  familyKey: FirstSliceReviewFamilyKey,
  x: number,
  y: number,
  ...
  return createContactObjects(scene, familyKey, x, y, direction, 'OPEN');
}
