...
export interface PrototypeRosterSlot extends PlayerRosterSlot {
  readonly acquisitionClass: AcquisitionClass; readonly rarity: PrototypeRarity; readonly seriesId?: string;
  readonly role: PrototypeRole; readonly description: string; readonly rewardScopes: readonly RewardScope[];
}
...
const SPECIAL_STAGE_CONTENT = parseCampaignStages(specialStagesJson, { playerUnitIds: ALL_PLAYER_UNIT_IDS, enemyIds: ENEMY_IDS, starterUnitId: STARTER_SLOT_ID, expectedStageCount: 5 });
...
const STAGE_POLICIES = parseStagePolicies([...stagePoliciesOneTwoJson,...stagePoliciesThreeJson,...stagePoliciesFourJson], new Set(BASE_STAGE_CONTENT.map((stage) => stage.id)));
...
function rosterSlot(unit: PlayerUnitContent): PrototypeRosterSlot {
  ...
  return { slotId: unit.id, displayName: unit.displayName, acquisitionClass: unit.acquisitionClass, rarity: unit.rarity, ...(unit.seriesId === undefined ? {} : { seriesId: unit.seriesId }), role: unit.role, description: unit.description, rewardScopes, definition: fighter(unit), cost: unit.cost, rechargeFrames: unit.rechargeFrames };
}
...
export function getContiguousClearedStageIds(clearedStageIds: readonly string[]): readonly string[] { const cleared = new Set(clearedStageIds); const contiguous: string[] = []; for (const stage of STAGES) { if (!cleared.has(stage.id)) break; contiguous.push(stage.id); } return contiguous; }
export function getUnlockedSlotIds(clearedStageIds: readonly string[]): readonly string[] { const cleared = new Set(getContiguousClearedStageIds(clearedStageIds)); const unlocked = new Set<string>([STARTER_SLOT_ID]); for (const stage of STAGES) if (cleared.has(stage.id) && stage.unlockUnitId) unlocked.add(stage.unlockUnitId); return PLAYER_SLOTS.filter((slot) => unlocked.has(slot.slotId)).map((slot) => slot.slotId); }
...
export function createPrototypeBattle(stageId = STAGES[0]!.id, unlockedSlotIds: readonly string[] = [STARTER_SLOT_ID], ownedRewardIds: readonly string[] = []): PlayableBattleState {
  const unlocked = new Set(unlockedSlotIds); const playerSlots = ALL_PLAYER_SLOTS.filter((slot) => unlocked.has(slot.slotId)); return createPrototypeBattleWithPlayerSlots(stageId, playerSlots, ownedRewardIds);
}
