...
export class StageSelectScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    addText(
      ...
      compact
        ...
        : `${this.collection.stages.length}개 전장 · 5개씩 보기 · ${this.collection.stageType === 'SPECIAL' ? '단계/메인 진도 조건 적용' : '순차 진도'}`,
      compact ? 22 : 19,
      COLORS.muted,
    );
    ...
    addButton(this, 72, 655, 115, compact ? 84 : 52, '◀ 이전', () => {
      this.page = Math.max(0, this.page - 1);
      this.renderPage();
    }, 0x586275);
    addButton(this, 1208, 655, 115, compact ? 84 : 52, '다음 ▶', () => {
    ...
    void loadGuestProgress().then((progress) => {
      ...
      if (this.requestedPage === undefined) {
        const firstUncleared = getFirstUnclearedCollectionStageIndex(
          this.collection,
          progress.clearedStageIds,
          progress.specialClearedStageIds,
        );
  ...
  private renderPage(): void {
    ...
    visible.forEach((stage, localIndex) => {
      ...
      const rewardOwned = !special
        && stage.permanentRewardId !== undefined
        && this.progress.permanentRewardIds.includes(stage.permanentRewardId);
      ...
      const border = unlocked
        ? (special ? (last ? 0xc28bcb : 0x80659b) : (last ? 0xbf9252 : 0x596c86))
        : 0x3c4554;

      this.stageLayer!.add(
        this.add.rectangle(x, 360, 220, 445, unlocked ? (special ? 0x2b2535 : 0x242b3a) : 0x1d222c, 0.98)
      ...
      if (!special && stage.unlockUnitId) {
        ...
        if (slot) {
          this.stageLayer!.add(addText(
            this,
            x,
            compact ? 445 : 503,
            compact ? `동료 · ${slot.displayName}` : `첫 클리어 동료 · ${slot.displayName}`,
            compact ? 19 : 14,
            cleared ? '#8ee3aa' : unlocked ? '#a8cfff' : '#59616d',
            'center',
          ).setOrigin(0.5));
        }
      }
  ...
  private showStageEnemies(stage: PrototypeStage): void {
    ...
    overlay.add(addButton(this, INTERNAL_WIDTH / 2, compact ? 575 : 570, 190, compact ? 84 : 58, '닫기', () => {
