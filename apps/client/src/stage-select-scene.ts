...
function getStageEnemyIds(stage: PrototypeStage): readonly string[] {
  const seen = new Set<string>(); const enemyIds: string[] = [];
  for (const wave of stage.waves) {
    if (seen.has(wave.spawn.enemyId)) continue;
    seen.add(wave.spawn.enemyId); enemyIds.push(wave.spawn.enemyId);
  }
  return enemyIds;
}
...
export class StageSelectScene extends Phaser.Scene {
  ...
  init(data: { collectionId?: string; page?: number } = {}): void {
    ...
    this.page = this.requestedPage ?? 0; this.enemyOverlay = undefined;
  }
  create(): void {
    drawBackdrop(this, 'map'); const compact = isCompactMobileViewport();
    addText(this, 54, 38, this.collection.title, 42, COLORS.cream);
    addText(this, 56, 91, compact ? `${this.collection.stages.length}개 전장` : `${this.collection.stages.length}개 전장 · 5개씩 보기 · ${this.collection.stageType === 'SPECIAL' ? '단계/메인 진도 조건 적용' : '순차 진도'}`, compact ? 22 : 19, COLORS.muted);
  ...
  private renderPage(): void {
    this.stageLayer?.destroy(true); this.stageLayer = this.add.container(0, 0);
    const compact = isCompactMobileViewport(); const special = this.collection.stageType === 'SPECIAL'; const start = this.page * 5;
    const visible = this.collection.stages.slice(start, start + 5); const discoveredEnemyIds = new Set(this.progress.discoveredEnemyIds ?? []);
    this.pageText?.setText(`${this.page + 1} / ${this.pageCount()}`);

    visible.forEach((stage, localIndex) => {
      const index = start + localIndex; const x = 145 + localIndex * 247;
      ...
      const rewardText = getPermanentRewardEffectText(stage.permanentRewardId); const last = index === this.collection.stages.length - 1;
      ...
      const stageEnemyIds = getStageEnemyIds(stage); const discoveredStageEnemyCount = stageEnemyIds.filter((enemyId) => discoveredEnemyIds.has(enemyId)).length;
      ...
      const stageButton = addButton(this, x, compact ? 535 : 548, 174, compact ? 84 : 52, unlocked ? (special ? '도전 시작' : '전투 시작') : compact ? '잠김' : lockedLabel, () => { if (unlocked) this.scene.start('battle', { stageId: stage.id }); }, unlocked ? border : 0x3f4855);
      if (!unlocked) stageButton.setAlpha(0.62); this.stageLayer!.add(stageButton);
    });
  }

  private showStageEnemies(stage: PrototypeStage): void {
    this.enemyOverlay?.destroy(true); const compact = isCompactMobileViewport(); const enemyIds = getStageEnemyIds(stage); const discoveredIds = new Set(this.progress.discoveredEnemyIds ?? []);
    ...
    if (enemyIds.length === 0) overlay.add(addText(this, INTERNAL_WIDTH / 2, 360, '출현 적 정보가 없다.', compact ? 24 : 20, '#8995a7', 'center').setOrigin(0.5));
    else {
      const columns = Math.min(4, Math.max(1, Math.ceil(Math.sqrt(enemyIds.length)))); const rows = Math.ceil(enemyIds.length / columns);
      const spacingX = compact ? 235 : 220; const spacingY = compact ? 126 : 112; const startX = INTERNAL_WIDTH / 2 - ((columns - 1) * spacingX) / 2; const startY = 330 - ((rows - 1) * spacingY) / 2;
      enemyIds.forEach((enemyId, index) => {
        const enemy = ENEMIES.find((candidate) => candidate.enemyId === enemyId); if (!enemy) return;
        const discovered = discoveredIds.has(enemyId); const boss = (enemy.definition.combatTags ?? []).includes('BOSS'); const col = index % columns; const row = Math.floor(index / columns); const x = startX + col * spacingX; const y = startY + row * spacingY;
