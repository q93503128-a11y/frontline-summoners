...
export class ResultScene extends BaseResultScene {
  ...
  private polishResultGeometry(): void {
    for (const child of this.children.list) {
      if (child instanceof Phaser.GameObjects.Text && child.text === '작전 결과') child.setAlpha(0.9);
      if (child instanceof Phaser.GameObjects.Rectangle && child.width > 1000 && child.height > 250) {
        child.setFillStyle(0x171e26, 0.9).setStrokeStyle(1, 0x6d7784, 0.45);
      }
    }
  }
}
