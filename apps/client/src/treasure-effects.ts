import type { CombatTrait } from '@frontline/sim';
import {
...
export type TreasureUnitStat =
  | 'maxHp'
  | 'attackDamage'
  | 'moveSpeed'
  | 'range'
  | 'rechargeFrames'
  | 'naturalKnockbackFrames';
...
export interface TreasureApplicableSlot extends PlayerRosterSlot {
  readonly role: string;
}
...
const KNOWN_TRAITS = new Set<CombatTrait>(['LIGHT', 'ARMORED', 'ARCANE', 'BOSS']);
const UNIT_STATS = new Set<TreasureUnitStat>([
  'maxHp',
  'attackDamage',
  'moveSpeed',
  'range',
...
const PERCENT_KINDS = new Set([
  'STARTING_SUPPLY_PERCENT',
  'PLAYER_BASE_HP_PERCENT',
  'SUPPLY_UPGRADE_COST_PERCENT',
  'ENEMY_REWARD_PERCENT',
]);
...
function parseSelector(value: unknown, context: string): TreasureUnitSelector {
  ...
  const selector: {
    ...
    trait?: CombatTrait;
    costAtMost?: number;
    attackMinRangeEquals?: number;
    attackMinRangeAtLeast?: number;
  } = {};
  ...
  if (value.trait !== undefined) {
    if (typeof value.trait !== 'string' || !KNOWN_TRAITS.has(value.trait as CombatTrait)) throw new Error(`${context}.trait is unknown`);
    selector.trait = value.trait as CombatTrait;
  }
...
function parseModifier(value: unknown, context: string): TreasureModifier {
  ...
  if (value.kind === 'PLAYER_UNIT_CAP_FLAT') {
    return { kind: value.kind, amount: requireInteger(value.amount, `${context}.amount`, -50, 50) };
  }
...
function selectorMatches(slot: TreasureApplicableSlot, selector: TreasureUnitSelector): boolean {
  ...
  if (selector.trait !== undefined && !(slot.definition.traits ?? []).includes(selector.trait)) return false;
...
export function applyTreasureBattleEffects<TSlot extends TreasureApplicableSlot>(input: TreasureBattleInput<TSlot>): TreasureBattleOutput<TSlot> {
  ...
  let playerUnitCap = input.playerUnitCap ?? 50;
  ...
  for (const treasureId of owned) {
    ...
    for (const modifier of effect.modifiers) {
      if (modifier.kind === 'STARTING_SUPPLY_PERCENT') {
        startingSupply = applyIntegerPercent(startingSupply, modifier.percent, 0);
      } else if (modifier.kind === 'PLAYER_BASE_HP_PERCENT') {
        playerBaseHp = applyIntegerPercent(playerBaseHp, modifier.percent);
      } else if (modifier.kind === 'PLAYER_UNIT_CAP_FLAT') {
        playerUnitCap = Math.max(1, playerUnitCap + modifier.amount);
      } else if (modifier.kind === 'SUPPLY_UPGRADE_COST_PERCENT') {
