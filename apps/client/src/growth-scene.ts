...
export class GrowthScene extends Phaser.Scene {
  ...
  private renderList(): void {
    ...
    visible.forEach((characterId, index) => {
      ...
      const button = addButton(this, 250, y, 350, compact ? 64 : 58, `${slot.displayName} · Lv${meta?.level ?? 1} +${meta?.plusLevel ?? 0}`, () => { this.selectedCharacterId = characterId; this.renderList(); this.renderDetail(); }, selected ? 0xc5a04c : 0x59677f);
      this.listLayer!.add(button); this.listLayer!.add(addText(this, 88, y - 24, badge, compact ? 16 : 13, color));
    });
  }
  private renderDetail(): void {
    this.detailLayer?.destroy(true); this.detailLayer = this.add.container(0, 0); const compact = isCompactMobileViewport(); const characterId = this.selectedCharacterId;
    ...
    const portrait = this.add.sprite(610, 245, art.family.idle.key, 0).setTint(art.tint); portrait.setScale((116 / art.family.idle.frameHeight) * art.displayScale); this.detailLayer.add(portrait);
    ...
    addText(this, 710, 155, slot.displayName, compact ? 31 : 29, '#ffffff'); addText(this, 712, 196, badge, compact ? 18 : 16, badgeColor); addText(this, 712, 230, `Lv${meta.level} / 현재 상한 Lv${levelCap} · +${meta.plusLevel}`, compact ? 20 : 17, '#f2d37c'); addText(this, 712, 266, slot.description, compact ? 17 : 15, '#b8c0ce').setWordWrapWidth(470);
    ...
    const forms = getEvolutionForms(characterId); addText(this, 930, 338, '형태', compact ? 23 : 21, '#e5c7ff');
    if (forms.length === 0) addText(this, 930, 392, '현재 선택 가능한 진화 형태가 없습니다.', compact ? 18 : 15, '#8995a7').setWordWrapWidth(300);
    else forms.forEach((form, index) => {
      const unlocked = meta.unlockedFormIds.includes(form.formId); const selected = meta.selectedFormId === form.formId; const label = unlocked ? `${form.formOrder}형태 · ${form.name}${selected ? ' · 선택 중' : ''}` : `${form.formOrder}형태 · 미해금`;
      const formButton = addButton(this, 1070, 398 + index * 76, 285, compact ? 66 : 58, label, () => { if (unlocked && !selected && !this.saving) void this.selectForm(characterId, form.formId); }, selected ? 0xc5a04c : unlocked ? 0x7b6990 : 0x46505e);
      if (!unlocked) formButton.setAlpha(0.55); this.detailLayer!.add(formButton);
    });
    this.detailLayer.add(addText(this, 830, 603, getNextCapMessage(levelCap), compact ? 17 : 14, levelCap >= 50 ? '#8ee3aa' : '#9ca9bb', 'center').setOrigin(0.5));
  }
