import type { EnemyWaveDefinition } from './playable.ts';
import type { BossRushEntry } from './record-playable.ts';
...
export const ENDLESS_RECORD_WAVES: readonly EnemyWaveDefinition[] = [
...
const RECORD_MODE_BY_ID = new Map(RECORD_MODE_DEFINITIONS.map((mode) => [mode.id, mode] as const));

export function getRecordModeDefinition(modeId: RecordModeId): RecordModeDefinition {
  const mode = RECORD_MODE_BY_ID.get(modeId);
  if (!mode) throw new Error(`Unknown record mode:${modeId}`);
  return mode;
}

export function isRecordModeId(value: string): value is RecordModeId {
  return (RECORD_MODE_IDS as readonly string[]).includes(value);
}

export function isRecordModeUnlocked(modeId: RecordModeId, clearedStageIds: readonly string[]): boolean {
  return clearedStageIds.includes(getRecordModeDefinition(modeId).unlockAfterStageId);
}
